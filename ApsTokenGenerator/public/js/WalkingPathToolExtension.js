...
(function () {
  ...
  class DrawWalkingPathLinesTool extends Autodesk.Viewing.ToolInterface {
    ...
    register() {
      ...
      this.snapper = new Autodesk.Viewing.Extensions.Snapping.Snapper(
        this.viewer,
    ...
    _updateCurrentMesh() {
      if (this.currentMesh) {
        this.viewer.overlays.removeMesh(
          this.currentMesh,
          DrawWalkingPathLinesOverlayName
        );
        this.currentMesh = null;
      }
      if (this.currentPoints.length > 0) {
    ...
    extractLastPathPoints() {
      return Array.isArray(this.lastPathPoints)
        ? this.lastPathPoints.slice()
        : [];
    }
  }
  ...
  class WalkingPathToolExtension extends Autodesk.Viewing.Extension {
    ...
    async load() {
      ...
      this.cameraTweenTool = await this.viewer.loadExtension(
    ...
    onToolbarCreated(toolbar) {
      ...
      drawWalkingPathButton.onClick = (ev) => {
        if (controller.isToolActivated(DrawWalkingPathLinesToolName)) {
          ...
          drawWalkingPathButton.setState(
      ...
      const walkNavButton = new Autodesk.Viewing.UI.Button(
        "walking-navigation-button"
      );
...
(function () {
  ...
  const CAMERA_TWEEN_ANIMATION_COMPLETED_EVENT =
    "AdnCameraTweenAnimationCompleted";
